...
contains
  ...
  subroutine extras_controls(id, ierr)
    ...
    s% other_adjust_mdot => my_adjust_mdot ! DEVINA NOV2022
  ...
  subroutine my_adjust_mdot(id,ierr)
	...
	use const_def
	integer, intent(in) :: id
	real(dp) :: f, w, Inertia, dot_P_spin, P_spin, v_esc, dm_a, dm_eq, dm_dubus
	real(dp) :: R_M, R_CO, R_LC
	integer :: j
	...
	Inertia = 2.0/5.0*b% m(2)*pow2(b% s_donor% xtra6) !g/cm**2
	dot_P_spin = b% s_donor% xtra4 !s/s
	P_spin = b% s_donor% xtra3 ! s
	...
	* ((b% period/(60d0*60d0*24d0)) ** (1.4_dp))
	...
	s% mstar_dot = s% mstar_dot + s% xtra8
...
! where it represented the base of the convective layer (different notation)
   ...
   names(28) = 'pulsar_wind_loss'
   vals(28) = s% xtra8
   names(29) = 'pulsar_spindown_L'
   vals(29) = s% xtra7
