...
! ***********************************************************************
   ...
   !1                /(s% omega_avg_surf/(2d0*pi)), f_conv
      ...
      real(dp) function acc_radius(b, m_acc, B_field_bool) !Calculates Sch. radius of compact object (or surface radius in case of NS) in cm
          type(binary_info), pointer :: b
          real(dp) :: m_acc, a, acc_radius_M
          real(dp) :: r_isco, Z1, Z2, eq_initial_bh_mass
          logical :: B_field_bool
          ...
          if (m_acc/Msun <= 2.50d0) then ! NS
              ...
              acc_radius = 12.5E5_dp ! in cm
              if (B_field_bool) then
                 if ((b% xfer_fraction == 0d0) .or. (abs(b% mtransfer_rate)==0d0)) then
                       acc_radius_M=1d99 
                 else
                      acc_radius_M = (pow_cr(2.0d0*pow2(pi) / b% s_donor% cgrav(1), 1.0d0/7.0d0) * &
                      pow_cr(pow6(acc_radius)/((b% xfer_fraction*abs(b% mtransfer_rate)) &
                      * sqrt(b% m(2))),  2.0d0/7.0d0) * pow_cr(b% s_donor% xtra5 ,4.0d0/7.0d0))/2
                 end if
                
                 if ( (acc_radius_M >= acc_radius) ) then
                    !write(*,*) 'R_M=', acc_radius_M, acc_radius, b% xfer_fraction, b% mass_transfer_beta
                    acc_radius = acc_radius_M
                 end if
              end if
		...
		eq_initial_bh_mass = b% eq_initial_bh_mass
          ...
          b% s_donor% xtra6 = acc_radius
      end function acc_radius
      
      subroutine pulsar_properties(binary_id,ierr)
         use const_def, only: clight
         type(binary_info), pointer :: b
         integer, intent(in) :: binary_id
         integer, intent(out) :: ierr
         real(dp) :: initial_P_spin, initial_P_spin_dot, initial_B_field, initial_am_NS
         real(dp) :: initial_omega, final_omega_NS, delta_am_ns, final_am_NS, dm_units, dot_omega_NS, moment_of_inertia
         real(dp) :: v_diff, dm_dubus, R_M, R_CO, R_LC
         ierr = 0
         call binary_ptr(binary_id, b, ierr)
         if (ierr /= 0) then
            write(*,*) 'failed in binary_ptr'
         ...
         if (b% model_number == 1 ) then
             b% s_donor% xtra3 = 1.0d0 !s ! initial P spin
             b% s_donor% xtra4 = 1.0d-15 ! s/s ! initial P spin dot
        
             moment_of_inertia = 2.0/5.0*b% m(2)*pow2(acc_radius(b, b% m(2),.false.)) 

             b% s_donor% xtra5 = sqrt(b% s_donor% xtra3*b% s_donor% xtra4 /(8.0d0*pow2(pi)*&
             pow6(acc_radius(b, b% m(2),.false.))*pow2(sin_cr(pi/4.0_dp))/(3.0_dp*pow3(clight)*moment_of_inertia)))
             
             
            R_M = acc_radius(b, b% m(2),.true.)
            R_CO = pow_cr((b% s_donor% cgrav(1)*b% m(2)/pow2(initial_omega)),1.0_dp/3.0_dp)
            R_LC = (clight/initial_omega)
               
            b% s_donor% xtra9 = R_M
            b% s_donor% xtra10 = R_CO
            b% s_donor% xtra11 = R_LC
            
         else
            initial_P_spin = b% s_donor% xtra3_old
            initial_P_spin_dot = b% s_donor% xtra4_old
            initial_B_field = b% s_donor% xtra5_old   
            
            ! from Dubus 1999
	    dm_dubus = 3.2d-9 * sqrt(b% m(2)/(1.4_dp*msun)) * ((b% m(1)/msun) ** -0.2) &
	    * ((b% period/(60d0*60d0*24d0)) ** (1.4_dp))
	    
            initial_omega = 2.0*pi/initial_P_spin
            moment_of_inertia = 2.0/5.0*b% m(2)*pow2(acc_radius(b, b% m(2),.false.)) 
            initial_am_NS = moment_of_inertia*initial_omega  
            ...
            ! RLO spin-up
            if ( ((b% xfer_fraction*abs(b% mtransfer_rate)*b% s_donor% dt) .gt. 0.0d0) &
            .and. (abs(b% mtransfer_rate/(Msun/secyer)) .ge. dm_dubus) ) then
            
               R_M = acc_radius(b, b% m(2),.true.)
               !b% xfer_fraction = 1 - b% mass_transfer_beta
              
                
               if ((R_M .lt. R_CO) .and. ((R_M .lt. R_LC))) then
                   ! Accretion phase
                   v_diff = sqrt(b% s_donor% cgrav(1) *  b% m(2) / pow3(acc_radius(b, b% m(2),.true.)))-&
                   sqrt(b% s_donor% cgrav(1) *  b% m(2) / pow3(R_CO))   
                   delta_am_NS = (1.0d0) *&
                   v_diff * pow2(acc_radius(b, b% m(2),.false.)) * &
                   b% xfer_fraction*abs(b% mtransfer_rate)*b% s_donor% dt
                   
                   !write(*,*) 'eff factor=', 1.0d-14/(b% xfer_fraction*abs(b% mtransfer_rate))
               
                   final_am_NS = (initial_am_NS+abs(delta_am_NS))
                   final_omega_NS = final_am_NS / (2.0/5.0*b% m(2)*pow2(acc_radius(b, b% m(2),.false.)))
            
                   b% s_donor% xtra3 = 2.0*pi/final_omega_NS ! Spin
                   ...
                   sqrt(b% s_donor% cgrav(1) *  b% m(2) / pow3(R_CO))
                   delta_am_NS = (0.001d0) *&
                   v_diff * pow2(acc_radius(b, b% m(2),.true.)) * &
                   (1-b% xfer_fraction)*abs(b% mtransfer_rate)*b% s_donor% dt
               ...
               ! TODO check braking law, decrease diff between P ini and P final
               dot_omega_NS = -(2.0_dp*pow2(initial_B_field)*pow6(acc_radius(b, b% m(2),.false.))*&
               pow2(sin_cr(pi/4.0_dp))*pow3(initial_omega))/(3.0_dp*pow3(clight)*moment_of_inertia)
               final_omega_NS = initial_omega+(dot_omega_NS*b% s_donor% dt)
      ...
      end subroutine pulsar_properties
      ...
      subroutine my_mdot_edd(binary_id, mdot_edd, ierr)
         ...
         if (b% m(2)/Msun > 2.50_dp) then ! M2 > 2.5 Msol for BHs
             ...
             mdot_edd_eta = b% s_donor% cgrav(1) * b% m(2) / (pow2(clight) * acc_radius(b, b% m(2),.false.))
         end if
      ...
      subroutine data_for_extra_binary_history_columns(binary_id, n, names, vals, ierr)
         ...
         if (b% point_mass_i == 0) then ! if there is no compact object then trapping radius is 0
           trap_rad = 0.0_dp
           accretor_radius = 0.0_dp
         else ! Begelman 1997 and King & Begelman 1999 eq. 1: accretor is star 2 
           trap_rad = 0.5_dp*abs(b% mtransfer_rate) * acc_radius(b, b% m(2),.false.) / mdot_edd
           accretor_radius = acc_radius(b, b% m(2),.false.)
         end if
        ...
        if (b% point_mass_i /= 2) then
         ...
         names(7) = 'P_NS'
         vals(7) = b% s_donor% xtra3
         names(8) = 'P_dot_NS' 
         vals(8) = b% s_donor% xtra4
         names(9) = 'B_NS'
         vals(9) =  b% s_donor% xtra5
         
         names(10) = 'R_M' 
         vals(10) = b% s_donor% xtra9 
         names(11) = 'R_CO' 
         vals(11) = b% s_donor% xtra10 
         names(12) = 'R_LC' 
         vals(12) = b% s_donor% xtra11
         names(13) = 'P_dot_NS_t'
         vals(13) = b% s_donor% xtra12
      ...
      integer function extras_binary_check_model(binary_id)
         ...
         integer, intent(in) :: binary_id
         integer:: i_don, i_acc
         real(dp) :: q, dm_dubus, dm_a, dm_eq, R_M, R_CO, R_LC, initial_omega, beta
         integer :: ierr
         call binary_ptr(binary_id, b, ierr)
         if (ierr /= 0) then ! failure in  binary_ptr
            return
	 ...
	 dm_dubus = 3.2d-9 * sqrt(b% m(2)/(1.4_dp*msun)) * ((b% m(1)/msun) ** -0.2) &
       ...
       dm_a = (b% m(2)/msun - initial_mass(2))
       ...
       if (b% point_mass_i/=0 .and. ((b% rl_relative_gap(1) .ge. 0.d0) &
         ...
         if (b% point_mass_i/=1) then
          ...
          initial_omega = 2.0*pi/b% s_donor% xtra3_old
          R_M = acc_radius(b, b% m(2),.true.)
          R_CO = pow_cr((b% s_donor% cgrav(1)*b% m(2)/pow2(initial_omega)),1.0_dp/3.0_dp)
          R_LC = (clight/initial_omega)
          ...
          ! accretion eff = 1-mass_transfer_beta
          if ((R_M .lt. R_CO) .and. ((R_M .lt. R_LC))) then
              ! Accretion phase
               b% mass_transfer_beta = beta
          else if ((R_M .ge. R_CO) .and. ((R_M .lt. R_LC))) then
              ! Propeller phase
              b% mass_transfer_beta = 0.9d0
          else if ((R_M .ge. R_CO) .and. ((R_M .ge. R_LC))) then
              ! Ejector phase
       ...
       call pulsar_properties(binary_id,ierr)
